...
class UAVEnv_(object):
    ...
    t_fly = 1
    t_com = 7
    delta_t = t_fly + t_com  # 1s飞行, 后7s用于悬停计算 - Trong chuyến bay, 7 giây cuối cùng được sử dụng để tính toán di chuột
    v_ue = 1    # ue移动速度1m/s
    ...
    def step(self, action):  # 0: 选择服务的ue编号 ; 1: 方向theta; 2: 距离d; 3: offloading ratio
        ...
        dis_fly = action[2] * self.flight_speed * self.t_fly  # 1s飞行距离 - khoảng cách bay
        ...
        if self.sum_task_size == 0:  # 计算任务全部完成
            ...
            reward = 0
            self.sum_task_size = self.sum_task_size - self.task_list[ue_id]
            step_redo = True
            ...
            self.e_battery_uav = self.e_battery_uav - e_server  # uav 剩余电量 - uav năng lượng còn lại
